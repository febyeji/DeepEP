...
// utils_fake_dep.cuh — drop-in replacement for utils.cuh with fake address
// dependencies injected on relaxed flag stores.
//
// Background
// ----------
// DeepEP uses st.relaxed.sys.global (st_relaxed_sys_global) to post head/offset
// flags after copying data into shared ring buffers.  In the PTX memory model,
// relaxed operations carry no ordering guarantee: a consumer on another GPU may
// observe the flag before seeing the data, even when there is a logical
// data-before-flag dependency in source order.
//
// Fake address dependency
// -----------------------
// A LOP3 instruction with LUT=0x00 always produces zero, but reads all its
// source operands.  ptxas cannot constant-fold this because the sources are
// runtime-opaque register values.  Adding the LOP3 result to the store
// address creates a dep chain:
//
//   prior_load → LOP3 (result=0) → IADD64 → store address
//
// The hardware cannot issue the store until its address is known, and the
// address computation depends on the LOP3, which depends on prior loads.
// This provides ordering equivalent to a fence without fence overhead.
//
// Overhead (SM86, RTX 3090): ~0.5% vs ~2.88% for fence.acq_rel.gpu.
// SASS confirmed: LOP3.LUT survives ptxas at -O3 (XOR self does not).
//
// Usage
// -----
// Compile with -DENABLE_FAKE_DEP to activate.  Without the flag, all fake
// dep code compiles away and this file is identical in behavior to utils.cuh.
//
// Call sites of st_relaxed_sys_global must pass EP_FAKE_DEP(val) as the
// third argument where val is any value already computed from prior loads:
//
//   // Before:
//   st_relaxed_sys_global(flag_ptr, new_head);
//   // After:
//   st_relaxed_sys_global(flag_ptr, new_head, EP_FAKE_DEP(new_head));

#include "exception.cuh"

// ============================================================
// Fake dep infrastructure
// ============================================================

namespace deep_ep {

/// Returns 0 via LOP3(LUT=0x00); ptxas cannot eliminate this because
/// `val` is a runtime register value.  Use the result as an offset added
/// to a store address to create an address dependency on `val`.
__device__ __forceinline__ uint64_t fake_dep_lop3_zero(uint32_t val) {
    uint32_t zero;
    asm volatile("lop3.b32 %0, %1, %1, 0, 0x00;" : "=r"(zero) : "r"(val));
...
/// Compute an address offset of 0 that depends on `val` at the register level.
/// Use as the `dep` argument to st_relaxed_sys_global.
#define EP_FAKE_DEP(val) deep_ep::fake_dep_lop3_zero((uint32_t)(val))
#else
#define EP_FAKE_DEP(val) ((uint64_t)0)
#endif

// ============================================================
// Everything below is identical to utils.cuh except:
//   st_relaxed_sys_global gains an optional `dep` parameter
// ============================================================
...
// Modified: accepts optional dep (LOP3 zero) to add to ptr for fake address dep.
// When dep == 0 (ENABLE_FAKE_DEP not set, or EP_FAKE_DEP unused), identical
// to the original.
__device__ __forceinline__ void st_relaxed_sys_global(const int* ptr, int val, uint64_t dep = 0) {
    const int* dep_ptr = reinterpret_cast<const int*>(reinterpret_cast<uint64_t>(ptr) + dep);
    asm volatile("st.relaxed.sys.global.s32 [%0], %1;" ::"l"(dep_ptr), "r"(val) : "memory");
}
